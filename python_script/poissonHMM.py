...
class poissonHMM(base._BaseHMM):
			...
			for i in range(0, self.n_components):
				for j in range (0, self.n_features):
					try:
						...
						print("[catch error] features: ", self.n_features, "lamdas: ", self.lamdas_[j,i], "X: ", X[t,j])
		return np.log(prob)
	...
	def predict_n_hospital(self, X, index=1):
		state = self.predict(X)[-1]
		prob = np.zeros((2, self.n_components))
		lamdas = self.lamdas_[index, :]
		prob[0,:] = self.transmat_[state, :]
		prob[1,:] = lamdas
		return prob
